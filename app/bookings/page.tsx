...
import { Car, CalendarIcon, Clock, Phone, MessageSquare, Star, CheckCircle, XCircle, AlertCircle } from "lucide-react"
            ...
            <Dialog open={showBookingForm} onOpenChange={setShowBookingForm}>
              ...
              <DialogContent className="max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>Schedule an appointment with a service provider</DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea id="notes" placeholder="Describe the issue or any special requirements..." rows={3} />
                  </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentBookings.map((booking) => {
                    const StatusIcon =
                      bookingStatuses[booking.status as keyof typeof bookingStatuses]?.icon || AlertCircle
                    const statusColor =
                      bookingStatuses[booking.status as keyof typeof bookingStatuses]?.color || "text-gray-600"
                    const statusBg = bookingStatuses[booking.status as keyof typeof bookingStatuses]?.bg || "bg-gray-50"
                      ...
                      <div key={booking.id} className="p-4 border rounded-lg hover:shadow-sm transition-shadow">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">with {booking.customerName}</p>
                          </div>
                          <Badge variant="outline" className={`${statusBg} ${statusColor} border-0`}>
