...
export function DashboardLayout({ children }: DashboardLayoutProps) {
    ...
    // Helper function to get background color class based on user role
    const getBackgroundColorClass = (role: string | undefined) => {
        switch (role) {
            case 'FREELANCE_MECHANIC':
                return 'bg-violet-50'; // Faint violet for mechanics
            case 'GARAGE_OWNER':
                return 'bg-blue-50'; // Faint blue for garage owners
            case 'SPAREPARTS_SHOP':
                return 'bg-amber-50'; // Faint amber for spare parts shops
            case 'ADMIN':
                return 'bg-emerald-50'; // Faint emerald for admins
            case 'CUSTOMER':
            default:
    ...
    const navigation = [
              ...
              ]),
        {
            ...
            icon: Star,
            current: pathname === (user?.role === 'CUSTOMER' ? '/reviews' : '/dashboard/reviews'),
        },
    ];

    return (
        <div className={`min-h-screen ${getBackgroundColorClass(user?.role)}`}>
            {/* Header */}
            <header
                className={`border-b ${getBackgroundColorClass(
                    user?.role,
                )}/95 backdrop-blur supports-[backdrop-filter]:${getBackgroundColorClass(
                    user?.role,
                )}/60 sticky top-0 z-50`}
            >
            ...
            <div className="flex">
                ...
                <aside
                    className={cn(
                        `w-64 ${getBackgroundColorClass(
                            user?.role,
                        )} border-r min-h-screen transition-transform duration-200 ease-in-out`,
                        'md:translate-x-0',
